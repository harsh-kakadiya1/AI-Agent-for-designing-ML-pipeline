...
\author{Harsh Kakadiya\inst{1} \and
        ...
        Jalpesh Vasa\inst{1}\thanks{Corresponding author. Email: jalpeshvasa.it@charusat.ac.in}}
...
without exhaustive search. Figure~\ref{fig:metaflow} shows the serpentine flow of processing stages, separating pipeline execution (left) from metadata-driven decisions (right).
...
Table~\ref{tab:metadata_heatmap} summarises key properties; highlighted cells (e.g., D2 imbalance, D3 high-dimensionality) trigger targeted modifier rules.
...
house-price prediction. The detailed scores in Table~\ref{tab:all_results} show that LightGBM performs best on D1 and D2, Decision Tree on D3, and XGBoost on D4, confirming the utility of metadata-guided pruning.
